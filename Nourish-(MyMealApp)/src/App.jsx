...
// --- Custom Styles (Fonts, Scrollbars, Animations) ---
const GlobalStyles = () => (
  <style>{`
...
const Widget = ({ children, className = "", title, icon: Icon, action, subtitle, theme }) => (
...
const WaterBottle = ({ currentOz, goalOz, onAdd }) => {
...
const WellnessTrends = ({ entries, theme }) => {
  const stats = useMemo(() => {
...
const CustomCalendar = ({ selectedDate, onSelectDate, onClose, theme }) => {
...
const SortDropdown = ({ value, onChange, theme }) => {
                ...
                {todaysEntries.length === 0 ? (
                    ...
                    todaysEntries.map((entry) => (
                        ...
                        >
                            ...
                            <button onClick={(e) => handleDelete(e, entry.id)} className="absolute top-2 right-2 p-2 opacity-0 group-hover:opacity-100 text-rose-400 hover:text-rose-600 transition-all"><Trash2 size={14}/></button>
                        </div>
                    ))
                )}
            </div>
...
// Full-Screen Welcome Component (Replaced OnboardingModal)
const WelcomeScreen = ({ onSave, theme, defaultName, defaultEmail }) => {
...
const StreakCelebration = ({ currentStreak, onClose, theme, userName }) => {
